               ...
               (let ((found (find states pairs
                 ...
                 (if found
                     ...
                     (let ((state (make-state)))
                       (push (cons state states) pairs)
                       (values state t)))))
             (convert (states)
               (multiple-value-bind (new-state new-p) (register-states states)
                 (when new-p
                   (let ((map nil))
                     ...
                     (dolist (item map)
                       (let ((to-state (convert (cdr item))))
                         (when (member nfa-accepting (cdr item))
                           (push to-state acceptings))
                         (push (make-transition new-state to-state (car item)) transitions)))))
                 new-state)))
      (let ((initial (convert nfa-initials)))
